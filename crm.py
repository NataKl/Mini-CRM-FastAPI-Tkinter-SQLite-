...
# Абсолютный путь к папке crm/ (рядом с этим файлом)
CRM_DIR = str(Path(__file__).parent / "crm")
...
def main() -> None:
    ...
    uvicorn.run(
        "crm_api:app",
        app_dir=CRM_DIR,      # uvicorn добавит crm/ в sys.path
        host=args.host,
        port=args.port,
        reload=args.reload,
        reload_dirs=[CRM_DIR] if args.reload else None,
    )
